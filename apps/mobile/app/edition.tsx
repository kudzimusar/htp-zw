...
const editions=["Global","Africa","Zimbabwe","Southern Africa","East Africa","West Africa","Central Africa","North Africa"];
const topics=["Public Health","Health Systems","Research","Policy","Health Business","Investigations","Global Health"];
  ...
  return (
    <Page title="Select Your Edition">
      ...
      <Section>
        ...
        <View style={styles.chips}>{topics.map((item)=><Chip key={item} active={selectedTopics.includes(item)} onPress={()=>toggle(item,selectedTopics,setSelectedTopics)}>{item}</Chip>)}</View>
      </Section>
      <Section>
