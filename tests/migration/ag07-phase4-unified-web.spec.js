...
test('Vercel intentionally builds apps/mobile as the root public Reader without /htp-zw dependency', () => {
  ...
  expect(config.buildCommand).toBe('node scripts/web/build-phase11-vercel.js');
  expect(config.buildCommand.length).toBeLessThanOrEqual(256);
  const phase11Build = fs.readFileSync('scripts/web/build-phase11-vercel.js','utf8');
  expect(phase11Build).toContain("APP_ENV: 'staging'");
  expect(phase11Build).toContain("EXPO_PUBLIC_HEALTHTIMES_SERVICE_MODE: 'staging'");
  expect(phase11Build).toContain("EXPO_PUBLIC_SUPABASE_URL: 'https://gcdohgbmqhqwydgaxrcr.supabase.co'");
  expect(phase11Build).toContain("EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY: 'sb_publishable_");
  expect(phase11Build).toContain("HEALTHTIMES_WEB_BASE_URL: ''");
  expect(phase11Build).toContain("['run', 'build:phase4:web']");
  expect(phase11Build).not.toContain("EXPO_PUBLIC_HEALTHTIMES_SERVICE_MODE: 'source-parity'");
