...
function articleBody(doc) {
  if (String(doc?.access_policy || '').toLowerCase() !== 'public') {
...
function renderContextPage(doc) {
  if(!doc) return null;
  const name=stripTags(doc.name || doc.slug || 'Archive');
  const canonical=doc.canonical_url || `${SITE_URL}${String(doc.path||'').replace(/^\//,'')}`;
  const items=Array.isArray(doc.items)?doc.items:[];
  const label=doc.kind==='category'?'Category':doc.kind==='tag'?'Legacy tag':'Author';
  const list=items.length?items.map(item=>{
    const date=item.published_at ? new Date(item.published_at).toLocaleDateString('en-GB',{year:'numeric',month:'short',day:'numeric',timeZone:'UTC'}) : '';
    return `<article class="v21-source-story"><div class="v21-source-story-body"><span class="eyebrow">${esc(label)}</span><h3><a href="${esc(item.canonical_url||'#')}">${esc(item.title||'HealthTimes story')}</a></h3><div class="story-meta">${item.author_name?`<span>${esc(item.author_name)}</span>`:''}${date?`<span>${esc(date)}</span>`:''}</div></div></article>`;
  }).join(''):'<p>No migrated stories are currently available for this context.</p>';
  const graph={
    '@context':'https://schema.org',
    '@graph':[
      {'@type':'Organization','@id':ORG_ID,name:'HealthTimes',url:SITE_URL},
      {'@type':'CollectionPage','@id':canonical+'#collection',name,url:canonical,isPartOf:{'@id':SITE_URL}},
...
<head>
  ...
  <meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
  <meta name="theme-color" content="#092b3a">
  <title>${esc(name)} — HealthTimes</title>
  <meta name="robots" content="${esc(doc.robots||'noindex,follow')}">
  <link rel="canonical" href="${esc(canonical)}">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="HealthTimes">
  <meta property="og:title" content="${esc(name)} — HealthTimes">
  <meta property="og:url" content="${esc(canonical)}">
...
<body data-page="migrated-context">
  <a class="skip-link" href="#main-content">Skip to content</a>
  <header class="site-header ag05-server-header"><div class="shell header-main"><a class="brand" href="/"><span class="brand-copy"><strong>HealthTimes</strong><small>Zimbabwe · Africa · Global</small></span></a><nav class="desktop-nav" aria-label="Primary"><a href="/">Home</a><a href="/archive.html">Archive</a><a href="/premium.html">Premium</a></nav></div></header>
  <main id="main-content" class="shell v21-archive-shell">
    <header class="page-hero"><span class="eyebrow">${esc(label)} · migrated context</span><h1>${esc(name)}</h1><p>This compatibility route preserves a historical HealthTimes context path while taxonomy curation remains separate.</p></header>
    <section class="v21-archive-grid" aria-label="${esc(name)} stories">${list}</section>
  </main>
  <footer class="site-footer"><div class="shell footer-bottom"><span>HealthTimes</span><span>Independent health and science journalism.</span></div></footer>
  <script src="/analytics.js" defer></script>
</body>
...
module.exports={esc,stripTags,sanitizeHtml,storageUrl,resolvedImage,structuredGraph,renderStoryPage,renderContextPage,renderSitemap,renderFeed,renderAdPreview};
