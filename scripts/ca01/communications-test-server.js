...
const discussion = require('../../api/discussion.js');
...
const tlsKeyPath = process.env.CA01_TEST_TLS_KEY || '';
const tlsCertPath = process.env.CA01_TEST_TLS_CERT || '';
...
const handler = async (req,res) => {
  try {
    ...
    if (url.pathname === '/api/discussion') return discussion(req,res);
    ...
    res.end(JSON.stringify({error:'CA-01 certification gateway failure'}));
  }
};
...
server.listen(port,'127.0.0.1',()=>process.stdout.write(`CA01_TEST_SERVER_READY scheme=${useTls?'https':'http'} port=${port}\n`));
