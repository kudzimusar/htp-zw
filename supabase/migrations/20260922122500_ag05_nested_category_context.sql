-- AG-05 nested legacy category compatibility routes.
-- The AG-04 corpus contains 36 distinct category paths / 429 occurrences,
-- including historical nested category paths. The final path segment of each
-- observed category path matches exactly one imported section identity.
...
begin
  if v_path ~ '^/category/.+/$' then
    v_kind := 'category';
    v_slug := regexp_replace(v_path,'^.*/([^/]+)/$','\1');
